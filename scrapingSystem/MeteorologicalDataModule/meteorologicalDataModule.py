...
class MeteorologicaldataScraping():
    def __init__(self, file_num, start_year, end_year, start_month, end_month, ken_name, ken_no_list, ken_block_list, md_url_list, md_item):
        ...
        self.user_select_md_item = md_item
    ...
    def MDOutput(self): ##気象データリストの返却
        if self.user_select_md_item == '気温':
            self.__createXl(self.temp_output_list)
        elif self.user_select_md_item == '相対湿度':
            self.__createXl(self.rh_output_list)
        elif self.user_select_md_item == '絶対湿度':
            self.__createXl(self.ab_hu_output_list)
        else:
            self.__createXl(self.temp_output_list)
    ...
    def __createXl(self, output_list):
        ...
        try:
            ...
            self.__xlWriting(output_list, sheet)

            change_cr_dir = getattr(settings, "MEDIA_ROOT", None)
            ...
            user_create_xl_data = FileManageData.objects.get(result_file_num = self.file_num)
            user_create_xl_data.create_file = middle_save_path ##ここでDBにファイルパスを保存
            user_create_xl_data.save()
            ...
            user_process = ProcessResultData.objects.get(result_file_num=self.file_num)
            user_process.file_create_status = middle_status
            user_process.save()
        except:
            traceback.print_exc()

    def __xlWriting(self, output_list, sheet):
        init_count = 0
        data_count = 0
        ...
        for scraping_data in output_list:
            init_count += 1
            data_count += 1

            if init_count == 1:
                ##ヘッダーを記入
                sheet['A' + str(xl_count)] = self.ken_name
                sheet['B' + str(xl_count - 1)] = '年'
                sheet['C' + str(xl_count - 1)] = '月'
                sheet['D' + str(xl_count - 1)] = self.user_select_md_item
            ...
            sheet['D' + str(xl_count)] = scraping_data
