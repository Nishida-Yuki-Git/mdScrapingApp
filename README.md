...
## システム開発の経緯
#### システム開発を始めたきっかけ
システムの基盤のアーキテクチャを自作のものを設計してみたかったこと、  
設計したアーキテクチャで開発をしたシステムを、自宅の物理サーバーで運用をしたかったこと、  
以上の理由から自作システムの開発を開始しました。  
#### 気象データ明細出力システムを開発したきっかけ
気象データ収集部分のロジックとして、学生の頃に作成した既存のものがありました。  
当時、ロジックを作成した理由として、簡単に色々な地域の気象データを取得して、  
大学の卒業論文を円滑に進めたい、という理由でした。  
上記から、このロジックをシステムにすれば、私と同じように気象データを、様々な期間と様々な地域で  
簡単に取得できたらいいなと考えている人の役に立つと考え、本システムの開発に至りました。  

## システム操作手順
本システムの画面を立ち上げて直後の状態では、
<img src="./readme_src/sosa1.png" width="700">
<br>
上記のように、セレクトボックの中身がない状態で、ボタン等の操作ができないようになっております。  
...
<br>
通常DjangoアーキテクチャはMVTという、Viewの部分に業務ロジックを記述する構成になっています。  
これでは、ViewがデータのIn/Outと業務ロジックの両方を兼任することになり、Viewが太ってしまうことになります。  
そのためApplication層を拡張して、ViewはデータのIn/Outに集中をして、業務ロジックをApplication層に  
...
画面表示処理と、業務ロジック等のデータ処理を同じサーバー上で実施することを避けました。  
...
#### アーキテクチャに時間をかける
業務ロジック開発に集中をして今後気軽に機能を拡張できるようにするため、「アーキテクチャ」と「開発」でフェーズを完全に分離しました。  
...
開始をしていなかったことが原因でした。  
SQLで直接トランザクションの開始クエリを投げて明示的に開始をすることで、即座に解決をすることができました。  
